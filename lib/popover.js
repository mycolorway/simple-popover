(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.simple['popover'] = factory(jQuery,
      SimpleModule);
  }
...
Popover = (function(_super) {
  ...
  Popover.prototype.refresh = function() {
    ...
    if (this.opts.offset) {
      if (this.opts.offset.vertical) {
        if (/top/.test(direction[0])) {
          top -= this.opts.offset.vertical;
        } else {
          top += this.opts.offset.vertical;
        }
      }
      if (this.opts.offset.horizental) {
        if (/left/.test(direction[0])) {
          left -= this.opts.offset.horizental;
        } else {
          left += this.opts.offset.horizental;
        }
      }

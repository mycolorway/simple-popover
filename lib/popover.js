(function() {
  ...
  Popover = (function(_super) {
    ...
    Popover.prototype.refresh = function() {
      var arrowHeight, arrowOffset, arrowWidth, bottomNotEnough, delta, direction, directions, left, pointToHeight, pointToHeightOffset, pointToOffset, pointToWidth, popoverHeight, popoverWidth, rightNotEnough, scrollLeft, scrollTop, top, winHeight, winWidth;
      ...
      if (this.opts.position) {
        ...
        pointToHeightOffset = (function() {
          switch (this.opts.verticalAlign) {
            case "top":
              return 0;
            case "bottom":
              return pointToHeight;
            default:
              return pointToHeight / 2;
          }
        }).call(this);
        bottomNotEnough = winHeight + scrollTop - pointToOffset.top - pointToHeightOffset < popoverHeight + 10;
      ...
      if (this.opts.autoAdjustPosition) {
        if (/top|bottom/.test(direction[0]) && left < scrollLeft) {
          ...
          switch (direction[1]) {
            case "left":
              this.arrow.css("right", parseInt(this.arrow.css("right")) + delta);
              break;
            case "right":
              this.arrow.css("left", parseInt(this.arrow.css("left")) + delta);
              break;
            default:
              this.arrow.css("marginLeft", parseInt(this.arrow.css("marginLeft")) - delta);
          }
        }
        if (/left|right/.test(direction[0]) && top < scrollTop) {
          ...
          switch (direction[1]) {
            case "top":
              this.arrow.css("bottom", parseInt(this.arrow.css("bottom")) + delta);
              break;
            case "bottom":
              this.arrow.css("top", parseInt(this.arrow.css("top")) + delta);
              break;
            default:
              this.arrow.css("marginTop", parseInt(this.arrow.css("marginTop")) - delta);
          }
        }
      }
      if (this.opts.offset) {
